...
import { AdminCohortComparisonTable } from "@/components/admin/admin-cohort-comparison-table"
...
const emptyRetakeComparison: AdminPerformanceAnalyticsPayload["retakeComparison"] = {
  firstAttempt: {
    attempts: 0,
    passed: 0,
    failed: 0,
      ...
      {!isLoading && !errorMessage && !hasData ? (
        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <AdminRetakeComparisonTable data={payload?.retakeComparison ?? emptyRetakeComparison} />
          </div>

          <AdminQuestionDifficultyTable data={payload?.questionDifficulty ?? []} />

          <AdminCohortComparisonTable
            data={
              payload?.cohortComparison ?? {
                groupBy,
                current: {
                  range,
                  rows: [],
            ...
            rangeLabel={payload?.range ?? range}
            compareRangeLabel={payload?.compareRange ?? compareRange}
          />
        </>
      )}
    </div>
